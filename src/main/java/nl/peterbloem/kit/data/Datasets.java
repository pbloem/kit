...
import nl.peterbloem.kit.distance.SquaredEuclideanDistance;
...
public class Datasets
{
	...
	/**
	 * Split the dataset in randomly chosen subsets
	 * 
	 * @param dataset
	 * @param split The proportion of the input that ends up in the first argument of the resulting pair.
	 * @return
	 */
			...
			List<P> dataset, double split)
	{
		...
		// nFirst += (int)(0.2 * nFirst); old code, no idea why this was here
	...
	/**
	 * Creates a generator for random points on a cube of the given 
	 * dimension. The radius is the radius of the inscribed sphere (ie. the distance
	 * from the origin to one of the sides of the cube)
	 */
	public static Generator<Point> cube(int dimension, double radius)
	{
		return new Cube(dimension, radius);
	}
	
	private static class Cube extends AbstractGenerator<Point>
	{
		protected int dim;
		protected double rad = 1.0;
		...
		public Cube(int dim, double rad)
		{
			this.dim = dim;
			this.rad = rad;
		}
		...
		public Point generate()
		{
			...
			for(int i = 0; i < dim; i++)
				vector[i] = Global.random().nextDouble()*(2 * rad) - rad;
	...
	/**
	 * The swiss roll: Four MVNs in 2D mapped to 3d with a smooth spiraling 
	 * function
	 * 
	 ...
	 * @throws IOException
	 */
	public static void writeCSV(List<Point> data, File file) throws IOException
	{		
		BufferedWriter writer = new BufferedWriter(new FileWriter(file));
		for(Point point : data)
	    {
			for(int i : series(point.size()))
				writer.write((i==0 ? "" : ", ") + point.get(i));
	...
	private static BufferedImage read(File file) throws IOException
	{
		...
		{
			throw new IllegalArgumentException("tiff images not (yet) supported.");
		}
